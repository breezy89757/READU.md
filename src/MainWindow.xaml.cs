...
";
    ...
    private static void ConfigureWebView(WebView2 wv)
    {
        ...
        // Map "readu.assets" to the local Assets folder so bundled JS/CSS
        // loads via https://readu.assets/js/... without any network access.
        var assetsPath = Path.Combine(AppContext.BaseDirectory, "Assets");
        wv.CoreWebView2.SetVirtualHostNameToFolderMapping(
            "readu.assets", assetsPath,
    ...
    {
        if (_activeTab is null || !_webViewReady) return;
    ...
    {
        if (wv is null || bodyHtml is null) return;
    ...
    private void OnSettingsChanged(object sender, ReadUSettings newSettings)
    {
        DispatcherQueue.TryEnqueue(async () =>
        {
            ...
            if (_activeTab is not null && _activeTab.Content is not null)
            {
    ...
    {
        if (_activeTab is null || _activeTab.IsWelcome) return;
        ...
        {
            // Entering edit mode
            if (_activeTab.Content is null && _activeTab.FilePath is not null)
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                DispatcherQueue.TryEnqueue(async () =>
                {
                    ...
                    TocItems.Clear();
                    if (_activeTab.Toc is not null)
                        foreach (var item in _activeTab.Toc)
                            TocItems.Add(item);

                    if (PreviewWebView?.CoreWebView2 is not null && _activeTab.RenderedHtml is not null)
                    {
    ...
    private async Task SaveActiveFileAsync()
    {
        ...
        try
        {
            ...
            if (_activeTab.Watcher is not null)
